          ...
          {projectData.map((data, index) => {
            return (
              ...
              <div className="bg-white w-full h-full mt-10 p-5 rounded-md space-y-2">
                ...
                {data.github && <Link href={data.github} className="text-sm flex items-center gap-1">Github <FaGithub /></Link>}
               {data.websiteURL && <Link href={data.websiteURL} className="text-sm flex items-center gap-1">Visit Now <CiGlobe /></Link>}
                </div>
